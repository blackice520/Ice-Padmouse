...
- **物理手柄**（蓝牙/USB，免 root）：可聚焦无障碍窗口捕获手柄按键**和模拟摇杆**
  ...
  - **L2（默认）= 打开/关闭游戏模式**（左扳机，任何状态下按一下切换）
...
针对"手游必须保持窗口焦点"的场景：游戏全程保持焦点，手柄按键**直接点击/滑动屏幕上的点位**，无需鼠标光标。按 **L2（左扳机）** 即可随时打开/关闭游戏模式。
...
- **新增「游戏模式开关」动作**：默认映射 R1（右肩键，数字键，任何状态可用）与 L2（左扳机，模拟轴，需鼠标激活）
- **修复 Xbox 手柄扳机失效**：L2/R2 只上报模拟轴（AXIS_LTRIGGER/RTRIGGER）、不产生 BUTTON_L2/R2 按键事件，新增模拟轴→数字键边沿翻译（顺带修复「R2=截屏」）
- **游戏模式开关 UI 同步**：切页重建 + 每秒轮询，手柄切换后 App 内开关状态即时跟上
- **游戏模式开关浮层提示**：改用无障碍浮层（TYPE_ACCESSIBILITY_OVERLAY）而非 Toast，任何 App/游戏上层都显示「已开启/已关闭」
